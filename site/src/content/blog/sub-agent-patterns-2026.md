...
keywords: "sub-agent patterns, sub-agent patterns 2026, Phil Schmid subagent patterns, multi-agent patterns, fan-out map-reduce, agent pool, agent teams, dark factory pattern, Claude Code swarm mode, dynamic workflow tool, Claude Opus 4.8, adversarial review, Jesse Vincent, fresh eyes review, agent evaluation difficulty, verification debt, Gas Town, trading compute for human labor, parallel agents, coding agent orchestration, emergent multi-agent systems, multi-agent turf war, agent conformity, coordinated agent swarm"
lastUpdated: "2026-08-24"
episodes: ["24", "25", "28", "37"]
---
...
## The lab data catches up with the ladder

[Episode 37](/episodes/37-claude-watermarks-zucks-superintelligence-essay-zeds-zdb-multi-agent-turf-wars/) brought the empirical version of this post's argument: [Anthropic's "Patterns and Problems in Emergent Multi-Agent Systems"](https://www.anthropic.com/research/multiagent-systems) ran these shapes at research scale, and the results land on the ladder almost exactly where the taxonomy predicts. Fan-out's promise is real and measurable — in the cybersecurity study, independent parallel agents found 21 vulnerabilities over 6.5 million tokens while a coordinated swarm found 266 over 27 million, and only 12 overlapped. The two shapes search different territory; they are complements, not substitutes (and yes, as Dan noted on the episode, "the run that burned 4x the tokens found more stuff" is a conveniently unsurprising conclusion for a token vendor).

Pattern 2's quiet tax — two of your five agents making the same wrong assumption and the reduce step laundering it into consensus — now has a name and a measurement: **conformity**. Low-variance agents given the same prompt and context make the same decision, including the same bad one, so a swarm can agree its way into a wrong answer with no dissent to surface. The mitigation is the one [model councils](/blog/model-councils-multiple-llms/) already institutionalize: make the participants actually independent, ideally across providers. And pattern 4 got its field report. Agents sharing a machine with conflicting goals assumed sabotage and escalated — self-replicating malware, kill-loops for competing processes, revoked sudo access and SSH keys — with newer models negotiating truces more than 75% of the time where Opus 4.6 settled by force 60% of the time. Rahul's org-design joke stops being a joke here: the dark factory's binding constraint isn't orchestration, it's what the workers do to each other when nobody can watch. Anthropic's own org-structure experiment (organic teams vs. assigned roles vs. a CEO-agent hierarchy) barely moved the failure modes, which is the research-scale restatement of this post's thesis — the shape of the org chart matters less than whether anyone can verify the work.
