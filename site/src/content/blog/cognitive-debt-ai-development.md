...
Four forces compound cognitive debt in ways that make it structurally different from previous forms of software understanding gaps.
...
**AI polish deletes the friction that flags the hard parts.** The subtlest compounding force came from an unexpected direction: Terence Tao's ICM talk ["Mathematics in the Age of AI"](https://teorth.github.io/tao-web/slides/age-of-ai-icm-2026.pdf), covered in [Episode 36](/episodes/36-pacing-the-frontier-anthropic-models-go-rogue-why-software-factories-fail-math-in-the-age-of-ai/). In a human-written proof, the parts the author found difficult retain natural friction — roughness that prompts the reader to slow down and pay attention. An excessively AI-polished proof removes that friction without replacing the signal, so the reader glides past exactly the passages that deserve scrutiny. The code version is immediate: a human's convoluted PR section historically came with an apologetic comment — "this is ugly, see these two files and the PRD constraint that forced it" — which told the reviewer where to dig. An agent's PR arrives uniformly confident, every section reading equally polished, or worse, with seven paragraphs on why the decision was correct. The attention-allocation signal reviewers relied on is gone precisely when, [per the same episode's Tech Week survey](/episodes/36-pacing-the-frontier-anthropic-models-go-rogue-why-software-factories-fail-math-in-the-age-of-ai/), line-by-line review is already down to one dev in ten — and it's hardest on juniors, who don't yet know that cognitive friction is how you learn where a system actually hurts.
