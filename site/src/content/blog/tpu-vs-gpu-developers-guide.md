...
Most developers who use AI daily have never thought about why the API call is fast or slow, cheap or expensive, available or waitlisted. The answer is almost always hardware. And the single most consequential hardware story in AI right now is not Nvidia's stock price — it is the fact that Google has been building its own purpose-designed chips since 2013, and those chips explain more about the competitive landscape than any [benchmark score](/glossary/benchmaxxed/) ever will.

When we covered this on [ADI Pod Episode 4](/episodes/4-open-ai-code-red-tpu-vs-gpu-and-more-autonomous-coding-agents/), my co-host Dan Lasky — who studied hardware engineering — walked through the physical architecture of TPUs versus GPUs in a level of detail that changed how I think about inference costs. This post distills that conversation into the parts that matter if you are a developer who consumes AI rather than trains it: what is actually different, why it matters for the models you use, and what it means for the choices you make about which APIs to call.

## How GPUs Ended Up Running AI
...
This is a pattern anyone who has worked in software recognizes: the technically adequate early solution becomes the industry default because switching costs compound faster than the performance gap widens. CUDA's moat is not raw compute — it is the ten years of libraries, tutorials, debugging tools, and muscle memory that sit on top of it.
...
What makes this more than a spec-sheet comparison is the timeline. Google started designing TPUs in 2013. The first version was released in 2016 — before OpenAI was even founded, and seven years before ChatGPT made LLMs a mainstream product. This was not a reactive move. Google's internal projections showed that if every Android user used voice search for just three minutes a day, the company would need to double its global data center capacity. Building the chip was a capacity planning decision, not an AI strategy decision. The AI advantage was a consequence.

## Why This Matters for Gemini (and for You)

Google's hardware advantage explains something that puzzled a lot of people in late 2025: why Gemini 3 was so fast. When we discussed it on the show, I pointed out that Google has their own chip, their own model, and some of the best engineers in the world — so how could they possibly lose this fight? The speed advantage is not primarily a model architecture story. It is a hardware story. Gemini runs on TPU infrastructure that was purpose-built for exactly this workload, and the systolic array architecture means inference involves fewer memory round trips per token generated.
...
GPUs are also more flexible. Nvidia's latest architectures include tensor cores that implement many of the same optimizations TPUs use for matrix math, while retaining the general-purpose programmability that makes GPUs useful for a wider range of workloads. If you need to run inference, train a model, and also run some non-ML compute on the same hardware, GPUs handle that. TPUs do one thing — they just do it exceptionally well.
...
The hardware layer is not something most application developers need to optimize directly. You are calling APIs, not provisioning silicon. But understanding the architecture underneath those APIs changes how you evaluate tradeoffs.
...
And when you hear that Google "fell behind" in AI and then suddenly "caught up" — understand that the hardware investment that makes Gemini competitive was started thirteen years ago. The AI race is not just a model architecture race or a data race. It is a silicon race, and the lead times are measured in years, not quarters.
