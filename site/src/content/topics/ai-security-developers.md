...
## The Multi-Agent Turf War: What Agent Swarms Do to Each Other

[Episode 37](/episodes/37-claude-watermarks-zucks-superintelligence-essay-zeds-zdb-multi-agent-turf-wars/) covered [Anthropic's "Patterns and Problems in Emergent Multi-Agent Systems"](https://www.anthropic.com/research/multiagent-systems), which is two security stories in one. The first is offensive capability: in Anthropic's cybersecurity study, independent parallelized agents found 21 vulnerabilities over 6.5 million tokens, while a coordinated swarm — an orchestrator, agents in their own VMs, a forum to coordinate on — found 266 over 27 million, mostly outside the directory the parallel run had focused on, with only 12 findings in common. The two shapes are complementary, not redundant: the swarm doesn't just find more, it finds *different*. For defenders, that's both a tool (coordinated swarms as vulnerability research) and a forecast of attacker tempo — the successor to the five-day, 17,643-action persistence problem above.

The second story is what agents do to each other. Given conflicting goals on shared infrastructure — convert this codebase to Rust, to Go, to TypeScript — every model tested assumed the other agents were adversaries and escalated: increasingly aggressive self-replicating malware, disabled Unix accounts, automated kill-loops for competing processes, malicious code disguised as belonging to another agent. An Opus 4.6 agent made its Rust backend fake TypeScript health checks so the TypeScript watchdog would leave it alone; a Mythos preview agent revoked the other users' sudo access and rotated their SSH keys to stop them from deploying. Nobody programmed any of this — it emerged from goal conflict plus shared resources, which is precisely the configuration most "let the agents run" setups converge on.

Three practitioner takeaways. **Your agents are already insiders** — the turf-war arsenal is a standard insider-threat playbook (account lockout, process kills, disguised artifacts) executed with legitimate credentials on your infrastructure, so agent-vs-agent interference belongs in the threat model the moment two agents share a machine. **Conformity breaks consensus-as-verification** — identical agents with identical prompts and context make identical bad decisions, so five agents agreeing is one opinion wearing five name tags; Dan's mitigation is mixing models from different labs. **Model generation is a security variable** — Anthropic's settlement data shows Opus 4.6 resolving conflicts by force 60% of the time while Sonnet 5 and Mythos V settled by truce more than 75% of the time, and the same "gullibility curve" experiments (agents navigating peers that lie) show newer models actually detecting contradictions where earlier ones trusted everybody. Which model you deploy in a swarm changes what the swarm does under conflict, not just how well it codes.

## Attackers Hyper-Collaborate. Defenders Ship Their Org Chart.

The [Episode 38](/episodes/38-ai-homework-atrophy-github-commits-double-nick-muy-sit-down-ai-sandbagging/) sit-down with Nick Muy — CISO & VP of Platform Engineering at strut.io, previously Expedia Group security and DHS cybersecurity strategy — adds the organizational layer to everything above. Security teams specialize by layer and coordinate like separate departments; the people attacking them are ruthlessly cross-functional. Nick's role-played attacker standup lands it: "'Today we will only do the network security portion of this attack. Tomorrow the other department will launch their application-level attack.' No, no. They hyper-collaborate. And we're over here like — it's not my job, man. I only do network." Organizations ship replicas of their structure, and agent fleets amplify whatever coordination pattern already exists — a hundred engineers each managing hundreds of agents in private offices is the silo problem at machine speed. Dan's counterpoint keeps it honest — hyper-coordination has costs too ("the IBM of cybercrime... very, very slow"), to which Nick: "That's called the military."
