...
### You Too Can Be a Middle Manager

[Episode 38](/episodes/38-ai-homework-atrophy-github-commits-double-nick-muy-sit-down-ai-sandbagging/)'s sit-down with Nick Muy (CISO & VP of Platform Engineering, strut.io) names what the flow-state mourning above turns into in practice: "Whether or not you asked for it, you too can be a middle manager." Watch a developer running a fleet of coding agents and the day is scoping, delegating, reviewing, and mediating — the first-line manager job description, with processes for direct reports. Nick's sharpest observation is who it's happening to: super-senior engineers who took IC roles specifically to avoid managing people, now doing management work all day at IC expectations and with IC support — none. He recognizes the feeling from his own first people-manager job ("What is this? What am I doing?"). The structural problem: the industry spent decades learning that first-time managers fail without support, then made every strong IC a first-time manager in roughly eighteen months, while engineering ladders still measure none of the skills that now predict throughput — scoping, delegation grain, verification discipline, knowing when to intervene. Nick's honest coda cuts both ways: on the comprehension problem of codebases growing ten million lines a week, "I was a lot more hopeful when I wrote this. Solutions? Not many" — and maybe the orchestration-maximalist direction is itself wrong: "go figure it out, take the guardrails off."

### Refinement Is the Durable Skill

[Episode 39](/episodes/39-metas-ai-backfires-glm-5-3-flash-openais-jalapeno-chip-the-end-of-programming/)'s Post Processing segment took on the essay everyone forwards with a subject line like "we're done": [Paul Dix's "The End of Programming"](https://pauldix.com/the-end-of-programming), anchored by the Bun 1.4 rewrite — one developer pointed Fable V at a million-plus lines of Zig and got a working Rust port in ~7,000 commits over 11 days, for roughly $165K in API costs. The part the subject line leaves out: it then took two months of agentic refinement before it shipped. "We're not talking about a lights-out AI dark factory for code." The career read is the one this page keeps arriving at from different directions: writing was never the durable skill — verification, direction-giving, and refinement are, and a port with a well-tested end state is precisely where AI is strongest today, while fuzzy greenfield criteria still fail. Rahul's honest counterpoint deserves the record: with longer-horizon agents and a known end state, even refinement may need less human in the loop a year from now. Dix himself expects "organizational inertia will likely mean another decade of humans writing code by hand" — and the same week gave that inertia a number. Meta's AI-native Project OT produced 220% more code changes, 36% more shipped features, 40% more incidents, and a cancelled second layoff round. File it next to the EY survey flip and Ford's grey-beard rehires: the wipeout keeps getting postponed by organizations discovering the bottleneck was never typing speed.
